...
> in Obsidian format. Built on Socrates' **maieutic**s (**산파술**): the skill
> interviews you to draw out the thinking already inside, instead of dumping
> critic-style answers on you.
>
> _Blog post conversion is handled by a companion skill, [`review-myblog-converter`](https://github.com/rhino-ty/review-myblog-converter)._
...
작품을 보고 그냥 '좋았어'에서 끝내지 않고, **사유를 끌어내는 인터뷰**를 통해 옵시디언에 깊이 있는 리뷰 노트를 작성한다.
...
> **블로그 변환은 별도 스킬**: 이 스킬에서 만든 노트를 네이버 블로그·티스토리·벨로그용 글로 변환하고 싶으면 [`review-myblog-converter`](https://github.com/rhino-ty/review-myblog-converter) 스킬을 사용하세요.

## 디렉토리 구조

```
polymedia-review-skill/
├── SKILL.md                    # 페르소나 + 워크플로우 + 평점 + 트리거
├── templates/
...
└── examples/
    ├── example-interview-flow.md    # KCD2 케이스로 보는 Phase 0~3 인터뷰 흐름
    ├── example-game-kcd2.md         # 모드 B로 작성된 완성 노트 예시
    └── example-anchor-game.md       # Anchor List 템플릿 (점수 흔들림 방지)
```
...
블로그용으로도 올리고 싶으면 → [`review-myblog-converter`](https://github.com/rhino-ty/review-myblog-converter) 로 옵시디언 노트를 입력.

### 실제 동작 예시

스킬이 어떻게 굴러가는지 보고 싶으면 [`examples/`](examples/) 참조:

- [`example-interview-flow.md`](examples/example-interview-flow.md) — 킹덤 컴 딜리버런스 2 케이스로 Phase 0~3 인터뷰 전체 흐름 압축
- [`example-game-kcd2.md`](examples/example-game-kcd2.md) — 모드 B로 작성된 완성 옵시디언 노트
- [`example-anchor-game.md`](examples/example-anchor-game.md) — Anchor List 템플릿 (점수 흔들림 방지)
...
| Language | Keywords |
|----------|----------|
| 🇰🇷 한국어 | 리뷰, 노트, 감상 정리, 평점, 별점, 다 봤어, 다 읽었어, 다 깼어, 옵시디언, 산파술 |
| 🇺🇸 English | review, write a review, just finished, just watched, just read, just played, obsidian note |
| 🇯🇵 日本語 | レビュー, 感想, ノート, 評価, 観終わった, 読み終わった, クリアした |
| 🇨🇳 中文 | 评论, 笔记, 感想, 评分, 看完了, 读完了, 通关了 |
