...
**종합 점수** = 모든 차원의 평균 (frontmatter `rating`에 자동 기록).
각 차원 점수는 본문에 **한두 줄 근거**와 함께 작성. 단순 별점이 아니라 *논증*이 되도록.

### 매체별 변형 — 허용되는 것

이 4D + 10점은 *기본 frame*이며, 매체 특성에 따라 다음이 가변이다:

1. **차원 자체** — 매체에 고유한 축을 *추가*하거나, 의미 없는 축을 *N/A*(`-`로 표기)하거나, 정밀한 이름으로 *명칭 변경* 가능
2. **차원의 해석** — 같은 `craft`라도 매체에 따라 의미가 다름 (책의 craft = 문장력, 음악의 craft = 사운드 디자인, 게임의 craft = 시청각·폴리시 등)
3. **척도** — 매체 특성상 더 정밀하거나 거친 평가가 필요하면 변경 가능 (10점 정수 / 5점 소수 등)
4. **N/A 환산 룰** — 어떤 축이 해당 없을 때 평균을 어떻게 낼지 (해당 축 제외 평균 / 가중치 환산 등)

**원칙**:
- 모든 변형은 *해당 매체 template 상단에서 self-document* — 무엇을 바꿨고 *왜* 바꿨는지 표로 명시
- 매체끼리 직접 비교가 필요 없으면(개인 노트는 보통 매체별로 분리됨) 변형이 자유로움
- 변형 후 점수는 *해당 매체 내에서*만 직접 비교 가능. 매체 간 비교는 frontmatter `rating`을 *정규화*해서

각 매체별 정의·변형은 `templates/*.md` 참조. 현재 적용 사례:
- **게임** → 5D(+`systems`) + 5점 소수 척도 + `rewatch`→`replay` 명칭 (자세한 이유는 `templates/game.md` 상단)
- 책·영화·음악 → 변형 없음 (기본 4D + 10점 유지)
